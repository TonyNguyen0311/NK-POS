...
def render_cost_allocation_page(cost_mgr, branch_mgr, auth_mgr):
    ...
    all_branches = branch_mgr.get_all_branches()
    ...
    with tab1:
        render_apply_allocation(cost_mgr, hq_branch_id, user_id)
...
def render_rules_management(cost_mgr, all_branches):
    ...
    with st.form("new_rule_form"):
        ...
        branch_id = form_cols[0].selectbox("Chọn chi nhánh", [b['id'] for b in all_branches if b['id'] != 'HQ'])
        percentage = form_cols[1].number_input("Tỷ lệ %", min_value=1, max_value=100, step=1)
        if form_cols[2].button("Thêm"):
        ...
        submitted = st.form_submit_button("Lưu Quy tắc mới")
        if submitted:
            if not rule_name:
    ...
    for rule in rules:
        with st.expander(f"{rule['name']} ({len(rule['splits'])} chi nhánh)"):
            ...
            if st.button("Xóa Quy tắc", key=f"del_rule_{rule['id']}"):
                cost_mgr.delete_allocation_rule(rule['id'])
                st.success("Đã xóa quy tắc.")
                st.experimental_rerun()

def render_apply_allocation(cost_mgr, hq_branch_id, user_id):
    st.subheader("Áp dụng Quy tắc vào Chi phí")
    
    # Lấy danh sách quy tắc
    rules = cost_mgr.get_allocation_rules()
    rule_options = {rule['id']: rule for rule in rules}
    if not rule_options:
        st.warning("Chưa có quy tắc phân bổ nào. Vui lòng tạo quy tắc ở tab bên cạnh trước.")
        return
    ...
    for cost in unallocated_costs:
        ...
        cols[1].write(f`{cost['amount']:,} VND`)
        ...
        if cols[3].button("Áp dụng", key=f"apply_{cost['id']}"):
